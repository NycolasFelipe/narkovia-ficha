...
const condutas = loadJsonFiles("condutas", files) as unknown as ContentGraduacao[];

class Conduta {
  static getItemById = function (id: string) {
    const item = getContentItem(condutas, id) as unknown as ContentGraduacao;
  ...
  static getItemsKeys = function () {
    ...
    $.each(condutas, (_index, item: ContentGraduacao) => {
      const id = item?.id;
      if (id) {
        keys.push(item.id);
      }
